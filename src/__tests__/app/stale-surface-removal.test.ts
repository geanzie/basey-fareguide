...
describe("Phase 6 stale surface removal", () => {
  it("removes deleted mock and legacy routes from the app tree", () => {
    ...
    expect(existsSync(repoPath("src", "components", "EnforcerDashboard.tsx"))).toBe(false);
    ...
    expect(existsSync(repoPath("src", "app", "api", "enforcer", "dashboard", "route.ts"))).toBe(false);
  ...
  it("keeps mounted admin and enforcer surfaces free of removed fake workflows", () => {
    ...
    expect(enforcerPage).not.toContain("EnforcerDashboard");
