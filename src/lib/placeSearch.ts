import type { Place, PlaceCoordinates } from '@/types/places';
...
function scoreOf(place: Place, query: string): number | null {
 ...
 */
function dropCoincident(places: Place[]): Place[] {
  const kept: Place[] = [];
...
export function searchPlaces(places: Place[], rawQuery: string): PlaceSearchResult {
  ...
  const near: Array<{ place: Place; distance: number }> = [];
  for (const place of places) {
 ...
 * the app — a fresh install with no recents — into the most useful one. With no
 ...
 */
export function browsePlaces(places: Place[], origin: PlaceCoordinates | null): Place[] {
